...
export class SaveNotePageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((pmap) => {
      ...
      this.getNotesSub = this.notesService.getSaveUserNotes({user_id: this.id}).subscribe(notes => this.notes = notes);
  ...
  retrieveNotes(): void {
    this.unsub();
    this.getNotesSub = this.notesService.getSaveUserNotes({user_id: this.id}).subscribe(returnedNotes => {
